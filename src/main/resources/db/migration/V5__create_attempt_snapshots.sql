...
CREATE TABLE quiz_attempt_allowances (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    created_at TIMESTAMPTZ NOT NULL,
    updated_at TIMESTAMPTZ NOT NULL,
    ...
    CONSTRAINT fk_quiz_attempt_allowances_user
        FOREIGN KEY (user_id) REFERENCES users(id)
        ON DELETE RESTRICT,

    CONSTRAINT fk_quiz_attempt_allowances_quiz
        FOREIGN KEY (quiz_id) REFERENCES quizzes(id)
        ON DELETE RESTRICT,

    CONSTRAINT uk_quiz_attempt_allowances_user_quiz
        UNIQUE (user_id, quiz_id),

    CONSTRAINT ck_quiz_attempt_allowances_remaining
        CHECK (remaining_attempts >= 0),

    CONSTRAINT ck_quiz_attempt_allowances_cooldown
        CHECK (cooldown_until IS NULL OR remaining_attempts = 0)
);
